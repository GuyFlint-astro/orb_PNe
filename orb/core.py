...
class Params(dict):
    ...
    def save(self, path):
        ...
        with orb.utils.io.open_hdf5(path, 'w') as f:
            for ikey in data:
                try:
                    # print(ikey, type(data[ikey]))
                    # print(data[ikey])
                    # print()
                    if ikey == 'fit_params' | ikey == 'fit_params_err':
                        print('the problem occurs here') ### NANCY EDITED THESE LINES (well, Guy really!)
                        continue
...
#################################################
#### CLASS Filter ############################### ### EDITED (added in) BY NANCY
#################################################
